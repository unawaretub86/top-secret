...
func (topSecretService *topSecretService) GetLocationAndMessage(body string, requestID string) (*entities.LocationMessage, error) {
	...
	satellites, err := request.ConvertToTopSecretRequest(body, requestID)
	if err != nil {
		return nil, err
	}

	// validamos que sean mas de 3 satelites
	...
	// a partir de las distancias recibidas de tres satélites: satelite[0], satelite[1] y satelite[2].
	x, y := topSecretService.triangulationUseCase.GetLocation(satelite[0].Distance, satelite[1].Distance, satelite[2].Distance)

	// Llama a la función GetMessage del caso de uso 'messageUseCase' para obtener el mensaje secreto.
	// Utiliza el 'requestID' para identificar el mensaje y las palabras recibidas de los satélites satelite[0], satelite[1] y satelite[2].
